...
Unknown query parameters are **ignored without error**, and so are some *known-looking* ones.
Confirmed:
...
`count` · `min_price` / `max_price` (**the span of the *unfiltered* query — they do not move when
...
- **(b) Send a single derived request header** `Cookie: deliver_city=<validated int>`, with **no cookie
  ...
  not carrying *Torob's* state, which this does not do.
...
status *before* parsing, and parse error bodies with a dedicated union of the three shapes above,
